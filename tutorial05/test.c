...
static void test_parse_array() {
    ...
    {
        lept_value*  vv = lept_get_array_element(&v, i);
        EXPECT_EQ_INT(LEPT_ARRAY, lept_get_type(vv));
        EXPECT_EQ_SIZE_T(i, lept_get_array_size(vv));
        for (size_t j = 1; j < lept_get_array_size(vv); j++)
        {
            lept_value* vvv = lept_get_array_element(vv, j);
            EXPECT_EQ_INT(LEPT_NUMBER, lept_get_type(vvv));
            EXPECT_EQ_DOUBLE((double)j, lept_get_number(vvv));
        }
    }
    lept_free(&v);
}
